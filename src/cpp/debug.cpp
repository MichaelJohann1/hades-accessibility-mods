...
//
// GATING: Debug keys turn on simply when a file named chaos.dat sits next to the
//         DLL (created by build-debug.ps1). There is no hash or size check — the
//         real gate is the ENABLE_DEBUG_KEYS compile flag, which public release
//         builds omit, so none of this debug-key code ships in a normal build.

// Tri-state: 0 = not checked yet, 1 = chaos.dat present, -1 = absent
...
static bool ValidateChaosFile()
{
    ...
    if (present) {
        Log::Info("DEBUG: chaos.dat present — debug keys enabled");
    }
    return present;
}
...
// Inner SEH-protected subtitle chunk loader (no C++ objects — avoids C2712)
static bool TryLoadSubtitleChunk(lua_State* L, const char* data, size_t size, const char* langCode)
{
    int savedTop = lua.gettop(L);
    __try {
        int loadResult = lua.loadbufferx(L, data, size, "@subtitles", nullptr);
        if (loadResult != 0) {
            const char* err = lua.tolstring(L, -1, nullptr);
            Log::Warn("[SUBTITLE] Failed to load subtitles: %s", err ? err : "unknown");
            lua_pop(L, 1);
            return false;
        }
        ...
        if (callResult != 0) {
            const char* err = lua.tolstring(L, -1, nullptr);
            Log::Warn("[SUBTITLE] Failed to execute subtitles: %s", err ? err : "unknown");
            lua_pop(L, 1);
            return false;
        }

        Log::Info("[SUBTITLE] Loaded subtitle data for language: %s (%zu bytes)", langCode, size);
        return true;
    } __except(EXCEPTION_EXECUTE_HANDLER) {
        Log::Warn("[SUBTITLE] LoadSubtitleData crashed: 0x%08X", GetExceptionCode());
        lua.settop(L, savedTop);
        return false;
    }
}
...
void LoadSubtitleData(lua_State* L)
{
    ...
    // Build path: <dll_dir>/subtitles/<code>.lua (C++ objects outside __try)
    std::wstring dir = PathResolver::GetInjectedDllDir() + L"\\subtitles\\";
    std::wstring path = dir;
    for (const char* p = code; *p; ++p) path += static_cast<wchar_t>(*p);
    path += L".lua";

    // Read file into memory
    std::ifstream file(path, std::ios::binary | std::ios::ate);
    if (!file.is_open()) {
        // Try English fallback if preferred language not found
        if (strcmp(code, "en") != 0) {
            std::wstring enPath = dir + L"en.lua";
            file.open(enPath, std::ios::binary | std::ios::ate);
            if (!file.is_open()) {
                Log::Info("[SUBTITLE] No subtitle files found in subtitles/ folder");
                return;
            }
            Log::Info("[SUBTITLE] Using English subtitle fallback (no %s.lua)", code);
        } else {
            Log::Info("[SUBTITLE] No subtitle files found in subtitles/ folder");
            return;
        }
    ...
    auto size = static_cast<size_t>(file.tellg());
    if (size == 0 || size > 8 * 1024 * 1024) {  // Max 8MB (subtitle files can be ~1MB)
        Log::Warn("[SUBTITLE] Subtitle file invalid size: %zu", size);
        return;
    }
    ...
    TryLoadSubtitleChunk(L, buffer.data(), size, code);
}
