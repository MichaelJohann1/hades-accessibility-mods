...
- **`scripts/generate_language_files.py`** (required for non-English) — generates `languages/{lang}.lua` files for 10 supported languages by combining game text translations with manual UI string translations. Without these files, non-English players see English text. Requires the game to be installed (reads HelpText SJSON from the game directory).
...
- **`scripts/parse_game_text.py`** (developer tool) — updates hardcoded boon/item descriptions in the Lua mods with authoritative game text from HelpText.en.sjson. Run this after adding new hardcoded descriptions or changing existing ones to ensure they match the game's data. Run with `--apply` to modify files in-place.
